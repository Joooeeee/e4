...
ROOM_monitor.on('connection', function(socket) {
  ...
  console.log('A EDA-monitor connected');
});
...
var EDARECEIVER = io.of('/receiver').on('connection', (socket) => {
  console.log('\nA receiver connected:\n' +
    ' host: ' + socket.handshake.headers.host.split(":").shift() + '\n' +
    ' ip  : ' + socket.handshake.address);
  var moment = require('moment');
  socket.on('data', (data) => {
    if (data.information) datastring = data.recordtime + ',' + data.type + ',' + data.information;
    else datastring = moment(new Date()).format("DD-MM-YYYY HH:mm:ss.SSS") + ',' + 'GSR' + ',' + '0';
    console.log('Current Time: ' + moment(new Date()).format("DD-MM-YYYY HH:mm:ss.SSS") + '\nCount:' + data.count + '\nString:' + datastring);
    var currentDate= moment(new Date()).format("DD-MM-YYYY")
    fs.appendFile('./asset/data/E4_'+currentDate+'.csv',
      datastring,
      function(err) {
        if (err) throw err;
      });
    ...
    if (data.type == 'GSR') {
      ROOM_monitor.emit('EDA', {
        data: [{
          median: 0,
          raw: (data.information ? data.information : 0)
        }]
      })
      console.log("send");
    }

    if (data.information > 0.99 && data.information != 0 && data.information != null) {
      console.log("trigger music value ============================================>" + data.information)
      EDARECEIVER.emit('trigger', {
        trigger: true
      });
    }
    //else EDARECEIVER.emit('trigger',{trigger:false});
  });
